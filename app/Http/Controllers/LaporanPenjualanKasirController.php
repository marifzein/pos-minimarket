...
class LaporanPenjualanKasirController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Paging 20 record sesuai request
        $reports = $query->paginate(20)->withQueryString();

        // Query untuk hitung total di bagian bawah (footer) berdasarkan rentang tanggal yang dipilih
        $totals = DB::table('transactions')
            ->select(
                DB::raw('SUM(cash) as total_cash'),
                DB::raw('SUM(card) as total_card'),
                DB::raw('SUM(voucher) as total_voucher'),
